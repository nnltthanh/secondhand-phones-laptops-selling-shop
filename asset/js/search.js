    ...
    for (i = 0; i < array.length; ++i) {
        ...
        <div id="${array[i].id}" class="col-6 col-sm-2-4 mb-12">
          <div  class="card p-3 shadow product-item position-relative">
              <a href=""><img class="card-img-top" class="img-product" src="${array[i].image}"  alt=""></a>
...
function showRatings() {
    ...
    for (let i = 0; i < itemRatings.length; ++i) {
        let productItem = Object.assign({}, products[i]);
...
function showSearchProduct() {
    ...
    var filteredWord = searchInput.name;
    var url = new URL(window.location.href);
    var params = url.searchParams;
    var keyword = params.get(filteredWord);
    keyWordContainer.innerHTML = `<p>Kết quả tìm kiếm cho: ${keyword}</p>`;
    var searchArray = [];
    products.forEach((product) => {
        productName = product.name.toLowerCase();
        if (productName.indexOf(keyword.toLowerCase()) > -1)
            searchArray.push(product);
    })
    showProduct(searchArray);
    showRatings();
}
